...
type checkPodAnnot func(client crclient.Client, v annotationChange, nodeName string) error
...
var interconnectPodAnnotations = map[string]checkPodAnnot{
	util.OvnPodAnnotationName: func(client crclient.Client, v annotationChange, nodeName string) error {
...
func (p PodAdmission) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (warnings admission.Warnings, err error) {
	...
	for _, key := range changedKeys {
		if check := p.annotations[key]; check != nil {
			if err := check(p.client, changes[key], nodeName); err != nil {
