...
var NoUDNServiceLabelError = fmt.Errorf("endpointSlice missing %s label", types.LabelUserDefinedServiceName)
...
func NewController(client clientset.Interface,
	...
	c := &Controller{
		...
		repair:        newRepair(serviceInformer.Lister(), nbClient), //, netInfo, nadLister),
	...
	klog.Infof("riccardo: done creating new service controller for network=%s", netInfo.GetNetworkName())
...
func (c *Controller) Run(workers int, stopCh <-chan struct{}, runRepair, useLBGroups, useTemplates bool) error {
	...
	klog.Infof("RICCARDO netINfo=%+v", c.netInfo)
...
// belong to the network the services controller is responsible for.
func (c *Controller) skipService(service *v1.Service, event string) bool {
	if util.IsNetworkSegmentationSupportEnabled() {
		serviceNetwork, err := util.GetActiveNetworkForNamespace(service.Namespace, c.nadLister)
		if err != nil {
			utilruntime.HandleError(fmt.Errorf("Failed to retrieve network for service %s/%s upon %s event: %w",
				service.Namespace, service.Name, event, err))
			return true
		}
		if serviceNetwork.GetNetworkName() != c.netInfo.GetNetworkName() {
			klog.Infof("riccardo: [onServiceAdd] controller network %s != service network %s: SKIP",
				c.netInfo.GetNetworkName(), serviceNetwork.GetNetworkName())
			return true
		}
	}
...
func (c *Controller) onServiceUpdate(oldObj, newObj interface{}) {
	...
	if err == nil {
		if c.skipService(newService, "update") {
			return
		}
...
func (c *Controller) onServiceDelete(obj interface{}) {
	...
	if c.skipService(service, "delete") {
		return
	}
...
func (c *Controller) queueServiceForEndpointSlice(endpointSlice *discovery.EndpointSlice) {
	serviceName, err := c.getServiceNameFromEndpointSliceUDNAware(endpointSlice)
	if err != nil {
	...
	c.queue.Add(FormatKeyForService(serviceName, endpointSlice.Namespace))
}
...
func ServiceControllerKey(endpointSlice *discovery.EndpointSlice) (string, error) {
	serviceName, err := getServiceNameFromEndpointSlice(endpointSlice)
	if err != nil {
		return "", err
	}
	return FormatKeyForService(serviceName, endpointSlice.Namespace), nil
}

func getServiceNameFromEndpointSlice(endpointSlice *discovery.EndpointSlice) (string, error) {
	return _getServiceNameFromEndpointSlice(endpointSlice, true)
}

func getServiceNameFromMirroredEndpointSlice(endpointSlice *discovery.EndpointSlice) (string, error) {
	return _getServiceNameFromEndpointSlice(endpointSlice, false)
}

func (c *Controller) getServiceNameFromEndpointSliceUDNAware(endpointSlice *discovery.EndpointSlice) (string, error) {
	if c.netInfo.IsDefault() {
		return getServiceNameFromEndpointSlice(endpointSlice)
	} else {
		return getServiceNameFromMirroredEndpointSlice(endpointSlice)
	}
}

func _getServiceNameFromEndpointSlice(endpointSlice *discovery.EndpointSlice, useKubeControllerLabel bool) (string, error) {
	if endpointSlice == nil {
		return "", fmt.Errorf("nil EndpointSlice passed to _getServiceNameFromEndpointSlice()")
	}

	label := discovery.LabelServiceName
	errTemplate := NoServiceLabelError
	if !useKubeControllerLabel {
		label = types.LabelUserDefinedServiceName
		errTemplate = NoUDNServiceLabelError
	}

	serviceName, ok := endpointSlice.Labels[label]
	if !ok || serviceName == "" {
		return "", fmt.Errorf("%w: endpointSlice: %s/%s", errTemplate, endpointSlice.Namespace,
			endpointSlice.Name)
	}
	return serviceName, nil
}

func FormatKeyForService(name, namespace string) string {
