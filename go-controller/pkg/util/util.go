...
func IsEndpointSliceForNetwork(endpointSlice *discoveryv1.EndpointSlice, network string) bool {
	if endpointSliceNetwork, ok := endpointSlice.Labels[types.LabelUserDefinedEndpointSliceNetwork]; ok {
		klog.Infof("riccardo: endpointSlice %s/%s, network=%s, wanted=%s",
			endpointSlice.Namespace, endpointSlice.Name, endpointSliceNetwork, network)
...
func GetEndpointSlicesEventHandlerForNetwork(handlerFuncs cache.ResourceEventHandlerFuncs, netInfo NetInfo) cache.ResourceEventHandler {
	...
	if IsNetworkSegmentationSupportEnabled() {
		...
		if netInfo.IsDefault() {
			klog.Infof("riccardo: GetEndpointSlicesEventHandlerForNetwork: netInfo.IsDefault")
			...
			klog.Infof("riccardo: GetEndpointSlicesEventHandlerForNetwork: netInfo.IsPrimaryNetwork")
			...
			filterFunc = func(obj interface{}) bool {
				if endpointSlice, ok := obj.(*discoveryv1.EndpointSlice); ok {
					...
					isForThisNetwork := IsEndpointSliceForNetwork(endpointSlice, netInfo.GetNetworkName())
					klog.Infof("riccardo: GetEndpointSlicesEventHandlerForNetwork: controller_network=%s, endpointSlice %s/%s, isDefault=%t, isMirror=%t, isForNetwork=%t",
						netInfo.GetNetworkName(), endpointSlice.Namespace, endpointSlice.Name, isDefault, isMirror, isForThisNetwork)
					// return !IsDefaultEndpointSlice(endpointSlice) && IsEndpointSliceForNetwork(endpointSlice, netInfo.GetNetworkName())
		...
		if filterFunc != nil {
			klog.Infof("riccardo: GetEndpointSlicesEventHandlerForNetwork: filterFunc != nil")
			eventHandler = cache.FilteringResourceEventHandler{
